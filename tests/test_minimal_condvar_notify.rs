mod common;
use common::assert_output_file;

#[test]
fn minimal_condvar_notify_generates_correct_dot_output_file() {
    assert_output_file(
        "./examples/programs/minimal_condvar_notify.rs",
        "dot",
        "./net.dot",
        "./examples/results/minimal_condvar_notify/net.dot",
    );
}

#[test]
fn minimal_condvar_notify_generates_correct_lola_output_file() {
    assert_output_file(
        "./examples/programs/minimal_condvar_notify.rs",
        "lola",
        "./net.lola",
        "./examples/results/minimal_condvar_notify/net.lola",
    );
}

#[test]
fn minimal_condvar_notify_generates_correct_pnml_output_file() {
    assert_output_file(
        "./examples/programs/minimal_condvar_notify.rs",
        "pnml",
        "./net.pnml",
        "./examples/results/minimal_condvar_notify/net.pnml",
    );
}
